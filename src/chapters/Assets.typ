...
#block(breakable: false)[
  == Using assets
  During play, a player can add as many of their fictionally appropriate assets to a dice pool as they'd like. Since they spent a #spec_char.pp to create the asset, there's no need to spend another to use it.
]

Assets belong to the character of the player that created them, and by default can't be included in anyone else's pool. An asset created to help another character belongs to that character and can't be used by the creator's character. Spend an additional #spec_char.pp to declare an asset to be open and usable by any character in the scene, including GMCs.
...
The player may spend an additional #spec_char.pp to keep the asset around through to the end of the session.

#block(breakable: false)[
  == Creating Assets
  There are several ways for players to create assets during play:
]
- A player can spend a #spec_char.pp and create a #spec_char.d6 asset with a name they come up with.
- Some SFX allow for stunts—assets that start with a #spec_char.d8 die rating.
- A player can create an asset greater than #spec_char.d6 and without spending a #spec_char.pp by making a test.
- When the narrative situation calls for it, the Storyteller may declare that a #spec_char.d6 asset is created for free as part of a successful test that wasn't made explicitly to create an asset.

#block(breakable: false)[
  === Test-Created Assets
  To create an asset by attempting a test, a player declares what their character does to create the asset and assembles an appropriate dice pool. The Storyteller rolls to set the difficulty, usually with something basic like #spec_char.d6#spec_char.d6.
]
...
The Storyteller usually calls for the roll, though it's just as valid for a player to ask for it. The difficulty for an asset test is set by rolling a straight #spec_char.d6#spec_char.d6 --- unless the Storyteller decides otherwise. A player's dice pool for the test is assembled from appropriate traits.

With a successful test, the PC gains some crucial information. If the PC spends a #spec_char.pp after succeeding at the test, they can bank the asset until they need it, give it to another character, or keep it for longer.
...
#block(breakable: false)[
  == Signature Assets
  Some assets are so iconic and inherent to the characters that they don't need to create them or spend #spec_char.pp to keep them for longer. Signature assets belong to characters and act like any other trait.
]
...
- #spec_char.d8 Something special and interesting, or that you have a strong connection to.
- #spec_char.d10 Something rare and potent, or that you are defined by.
- #spec_char.d12 Something truly unique and powerful, or that transcends even your innate abilities.
...
Signature assets cannot be shared between PCs; if someone “borrows” a signature asset from a character (picking up the other character's heirloom sword, driving their custom sports car or referencing their grimoire), the borrower must spend a #spec_char.pp to create an asset as normal to get any benefit from the asset borrowed, and the asset is rated at #spec_char.d6 like any other #spec_char.pp created asset.
