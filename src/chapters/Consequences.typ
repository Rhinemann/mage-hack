...
#block(breakable: false)[
  == Complications
  A complication is an unhelpful temporary trait that the opposition includes in their dice pool to make the opposed roll harder for the character with the complication. Complications are given descriptive names and a die rating, starting at #spec_c.d6.
]
...
#block(breakable: false)[
  === #spec_c.d4 Complication
  If a complication is stepped down to a #spec_c.d4, the very next time the complication might affect your character, you add it to your own dice pool and earn a #spec_c.pp (much like a distinction being used with the Hinder SFX).
]
...
#block(breakable: false)[
  === Inflicting Stress
  Whenever a character fails an action that causes harm as a consequence they receive a #spec_c.d6 die of appropriate stress. Being a target of a successful attack inflicts stress equal to the effect die of the attack. Some SFX can also inflict stress upon a character.
]
...
#block(breakable: false)[
  ==== Stressed Out
  If any stress die rating is ever stepped up past #spec_c.d12, the character is taken out (or stressed out) and no longer takes part in the scene. By default, you can't spend a #spec_c.pp to delay this effect, though certain SFX may allow to do that. When you're stressed out, you are assumed to have #spec_c.d12 stress for the purposes of taking any additional stress, even though you can no longer act in the scene.
]
...
Stress rated at #spec_c.d4 functions just like a complication rated at #spec_c.d4; it goes into a player's dice pool instead of being added to the opposition dice pool and earns the player a #spec_c.pp. Right after that test or contest, it either goes away or — if the player rolls a hitch on one of their dice — gets stepped up as the injury gets worse.

Only one type of stress can be used against a character at any given time, unless the Storyteller pays the player a #spec_c.pp to add an additional stress die to the opposition dice pool. Characters can be affected by both stress and complications at the same time, however.
...
- If the PC beat the difficulty but rolled a hitch, the Storyteller may hand over a #spec_c.pp and introduce a new stress or complication related to the one that was just recovered.

- If the PC failed to beat the difficulty and rolled a hitch, the stress is stepped up by one step for every hitch rolled.

#block(breakable: false)[
  == Trauma
  Trauma is long-term stress. Any time a PC's stress is stepped up past #spec_c.d12, they're stressed out of the scene, and they gain #spec_c.d6 trauma of the same type of stress that just increased. Trauma functions just like stress but is much harder to recover.
]
...
• If you beat the difficulty but roll a hitch, the GM may hand over a #spec_c.pp and introduce either a complication that will hinder the PC in the next scene or inflict stress of a different type than the trauma that was being recovered, starting at #spec_c.d6 (or stepping up by one if the PC already had stress of that type).
• If you fail to beat the difficulty and roll one or more hitches, the trauma gets worse, stepping up by one for each hitch rolled. If this steps the trauma up past #spec_c.d12, that's all, folks.
